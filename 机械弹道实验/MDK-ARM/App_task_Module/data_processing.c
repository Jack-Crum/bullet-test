...
int	shoot_value = 120;                    //摩擦轮转速设定，设定值可设为：110—125
/* 内部函数原型声明-----------------------------------------------------------*/
...
void RemoteControlProcess()  
{
		switch(RC_Ctl.rc.s2)
		{
			case 1: //上
			{
				shoot_value = 126;  // 126为25.2 m/s
			}break; 
			case 2: //下
			{
				shoot_value = 112;  //  112为11~11.7m/s
			}break; 
			case 3: //中
			{
			  shoot_value = 115; //  115为14.6m/s
			}break; 
			default :break;
		}					
// 120为18m/s  113为12.4m/s  115为14.5  110为10m/s  
	  if(RC_Ctl.rc.s1 == 1)            //上
		{    
